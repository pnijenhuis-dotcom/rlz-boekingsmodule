...
| Bank/afletteren (voorstel-volgorde 1–5, niet door klant-accorderingsflow; G-rekening-split = standaardcase) | goedgekeurd → fase 2; **STAP-0-kanttekening over stap 4 HERZIEN door de schrijf-PoC (2026-08-02): RLZ's eigen voorstel bestaat wél (`MatchedPaymentItem` auto-gevuld bij exacte bedrag-match) — stap 4 blijft dus in het ontwerp, mét bronvermelding. Nieuwe kanttekening: stap 1/2 (auto-/deel-afletteren tegen open post) kan via de publieke API nog niet (actie 15/16-payload ongedocumenteerd, supportvraag opgesteld); direct-op-grootboek kan wél** | mockup `#bank`/`#bankdetail`; CLAUDE.md "Bank"; BOUWPLAN fase 2; verkenning/api-verkenning.md "Bankmodule schrijf-PoC" |
