...
| IBAN-wissel vier-ogen-accordering (accordeur-set per administratie, leeg → beheerders; aanbieden zet document op `wacht_op_iban_accordering` en blokkeert boeken; accorderen alleen door accordeur ≠ aanvrager → IBAN vertrouwd + herkomst hersteld; afwijzen = verplichte reden, document blijft geblokkeerd/verdacht; audit op alle drie) | **goedgekeurd (Cowork-mockup 2026-07-15, akkoord Peter); PART A backend + tests gebouwd 2026-07-15, PART B (UI) open.** Bewuste keuzes PART A: direct-bevestig-endpoint verwijderd (omzeilde vier-ogen), opnieuw aanbieden na afwijzing toegestaan (geen doodlopende status) | `docs/ontwerp/iban-wissel-accordering.md` (canonieke flow — geen mockup-bestand); `app/documenten/iban_accordering.py` + migratie 0024 |
