...
| P-25 | Betaalstatus naar Odoo: geen niet-afsluitend equivalent van RLZ's `QuickPaymentSelection` (STAP-0 08-09, `verkenning/odoo_stap0_betaalstatus.py`); volgende stap = schrijftest `account.move.line.blocked` op een TEST-bill (company 1) + toets of Odoo's betaallijst hem overslaat; tot dan zichtbare tijdlijnmelding, boeking loopt door | 2026-09-08 | sectie "RLZ-BETAALSTATUS INKOOPFACTUUR + INTAKE-KANAAL DECLARATIES", alinea Parkeerpost |
| P-26 | `webhook-herzenden`: referenties komma-gescheiden in één `--referentie` (gcloud `--args` weigert een herhaalde waarde → nu één job-executie per referentie) | 2026-09-24 | sectie "WEBHOOK-HERZENDEN — 11 KOSTENEVENTS VASTLY (OPEN_ITEMS regel 13, 23-09)", alinea 5 "Parkeerpost P-26" |
