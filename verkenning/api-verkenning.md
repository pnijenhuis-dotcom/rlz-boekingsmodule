...
`ManualJournals/{id}/Lines: 404 <html>…`, 0 van 3.718 journaalregels trof via `JournalEntry.EventID` een bekend document,
en élke inkoopfactuur viel terug op `Date` ("BookDate ontbreekt"). Alle calls hieronder liepen als Cloud Run-job-executie
(`rlz-reconciliatie`, image van `efe3fb7`), geanonimiseerd (GUID's → 8 tekens). Status + eerste tekens van het antwoord
LETTERLIJK:

| # | Route | Antwoord |
|---|---|---|
| a | `GET ManualJournals/{id}/Lines` (zonder `$expand`, `$top=5`) | **404**, HTML-body: `<html>\n<head>\n\t<style>\n\t\tbody {\n\t\t\tmargin: 0;\n\t\t\tbackground-color: #fff;\n\t\t\tfont-family: Arial, Helvetica, sans-serif;\n\t\t}\n\t\t.topbar {…` (RLZ's eigen 404-pagina, geen JSON) |
| b | `GET ManualJournals/{id}/Lines?$expand=Account,Project` | **404**, identieke HTML — de hypothese "TaxRate is geen geldige navigatie op memoriaalregels" is FOUT: **de route bestaat niet.** De Help-lijst (`verkenning/output/help.html`) kent voor ManualJournals alleen `ManualJournals`, `/{id}`, `/{id}?autoCorrect=`, `/{id}/Actions`, `/{baseId}/Uploads(/{id})`, `/{id}/DocumentTaskHistory`, `/{id}/QuickPaymentSelections`, `/Actions`, `/Totals`; `…/Lines`-routes bestaan alleen voor PurchaseInvoices, SalesInvoices, ProformaInvoices, Offerings en Forecasts |
| c | `GET ManualJournals/{id}?$expand=DocumentLineList($expand=Account)` via `rlz-lezen` | **400** `{"Message":"The query specified in the URI is not valid. The requested resource is not a collection. Query options $filter, $orderby, $count, $skip, and $top can be applied only on collections."…}` — veroorzaakt door de `$top=5` die `rlz-lezen` altijd meegaf: het instrument kon een RECORD niet lezen. **Gefixt in blok 7c:** een recordpad (laatste segment GUID) krijgt alleen `$expand` mee (`app/rlz/lezen_cli.py::is_recordpad`). De document-vorm zelf is voor ManualJournals dus NOG NIET live gelezen; bewezen is hij voor `SalesInvoices/{id}` ("Receipts-verkenning" §1) en `BankMutationDirectBookings/{id}` ("Bankmodule schrijf-PoC"). De replay-nameting ná deploy is de meetlat (0 documenten zonder regels) |
| c' | `GET ManualJournals?$filter=id eq {guid}&$expand=JournalEntryDiary,DocumentLineList($expand=Account)&$top=1` | **200**, één rij ZÓNDER de sleutel `DocumentLineList` — **de collectie-vorm van `DocumentLineList(…)` wordt stil genegeerd** (zelfde gedrag op PurchaseInvoices, zie hieronder; blok 7b's OVERGESLAGEN-regel "gaf geen DocumentLineList" op alle drie de collecties is dus geen uitzondering maar de regel). Kop-velden: `id, BalanceAmount 0.0, BaseInvoiceAmount 70.0, BaseRemainingAmount 0.0, TotalNetAmount 70.0, TotalPaidAmount 70.0, TotalPayableAmount 70.0, TotalTaxAmount 70.0 (⚠️ = netto, geen btw-veld om op te vertrouwen), BookDate "2025-07-15T00:00:00", Date "2025-07-15T00:00:00", Description "Aandelenkapitaal 70 procent Vast\ngoedgroep Nederland" (32-tekens-knip), PaymentReference null, ReceiptNumber "RLZ-06-00000001", Reference null, Status 3, DocumentType 11, DueDate, OverdueDays null, JournalEntryDiary {Code "MEMO", Description "Systeemboek voor Algemene Memoriaalboekingen", JournalEntryDiarySystem 0}` — **de ManualJournals-collectie draagt wél `BookDate`** |
| d1 | `GET JournalEntryLines?$filter=JournalEntry/EventID eq {guid}&$expand=JournalEntry,Account` | **400** `{"Message":"A binary operator with incompatible types was detected. Found operand types 'Reeleezee.DTO.JournalEvent' and 'Edm.Guid' for operator kind 'Equal'."}` — `EventID` is van het type `JournalEvent`, geen GUID |
| d3 | `…$filter=JournalEntry/EventID/id eq {guid}` | **400** `{"Message":"Can only bind segments that are Navigation, Structural, Complex, or Collections. We found a segment 'EventID' that isn't any of those."}` — geen navigatie mét id |
| d4 | `GET JournalEntryLines?$expand=Account,JournalEntry($expand=EventID)&$top=3` | **200**. Regelvelden: `id, CreditAmount, DebitAmount, VatAmount (0.0), Description, Account{id, IsDepartmentRequired, IsEmployeeCurrentAccount, IsEmployeeRequired, IsFixedAssetAccount, IsProductRequired, IsProjectRequired, IsTopLevel, IsTotalAccount, UseForBankCashMutationDetails, UseForManualJournalDetails, UseForPaymentAccount, UseForPurchaseInvoiceDetails, UseForSalesInvoiceDetails, IndentLevel, AccountantNotes, AccountNumber, Comment, Description, ExternalGeneralLedgerNumber, AccountType}, JournalEntry{id, BookDate, DocumentType, EventID}`. **`EventID` = klein geheel getal: 51 bij DocumentType 10, 71 bij DocumentType 1 — een SOORTCODE (JournalEvent), GEEN document-id.** Voorbeeld: `CreditAmount 1000.0 / 8105 Opbrengst verhuur "HuurBordeauxdreef 79, Rotterdam"` + `DebitAmount 1000.0 / 1001 ING "Onbekende klant"`, JournalEntry `{BookDate 2025-07-22, DocumentType 10, EventID 51}` = een entity-loze Receipt |
| d5 | `GET JournalEntries?$expand=EventID&$top=3` | **200** `{id, BookDate "2025-01-01T00:00:00", DocumentType 1, EventID 71}` ×3 — een JournalEntry draagt géén documentverwijzing; onbekende expands worden stil genegeerd |
| d8 | `GET JournalEntryLines?$filter=JournalEntry/id eq {memoriaal-guid}&$count=true` | **200**, `@odata.count 0` — **`JournalEntry.id` ≠ document-id** |
| d9 | `GET JournalEntries?$filter=id eq {memoriaal-guid}&$expand=Document,SourceDocument,Lines,JournalEntryLines,PaymentTransaction,JournalEntryDiary&$count=true` | **200**, count 0 |
| e | `GET PurchaseInvoices?$filter=ReceiptNumber eq 'RLZ-04-00000094'&$expand=JournalEntry,JournalEntries,Journal,Document,PaymentTermList,TaxSummaryList&$top=1` | **200**, rij mét ALLEEN de kale kop: `id, BaseInvoiceAmount 105423.53, BasePaidAmount, BaseRemainingAmount 0.0, TotalPayableAmount, Date "2025-06-01T00:00:00", Description "t", InvoiceReference "Ingescand document", ReceiptNumber, Reference, Status 3, DocumentType 1, Origin 1, Type 1, DueDate "2025-07-01T00:00:00", OverdueDays null` — **géén `BookDate` op de PurchaseInvoices-COLLECTIE** (de document-vorm `PurchaseInvoices/{id}` geeft 'm wél: "Boekingsdatum = BookDate — STAP 0 (28-08)" las `d.get('BookDate')` van precies die route) en géén JournalEntry-navigatie (alle zes expands stil genegeerd). Mét `$expand=Entity,DocumentLineList($expand=Account,TaxRate)`: Entity komt mee, `DocumentLineList` NIET |
| f | `GET JournalEntryLines?$filter=Account/AccountNumber eq '1700' or … '1300' or '1707' or '130' or '170'&$count=true` | **200**, `@odata.count 0` — **er staat in de hele VGG-administratie geen enkele journaalregel op een btw-grootboek.** De "883 regels mét btw" van blok 7 telden regels mét een `TaxRate`-verwijzing (0 %/"Geen BTW") óf TaxAmount ≠ 0 (`git show 028202e~1:backend/app/migratie/vertaling.py`, regel 379); blok 7b telt alleen `TaxAmount ≠ 0` → 0. De 7b-teller is juist; het rapport toont sinds 7c beide bronnen |
| g | `GET JournalEntryLines?$filter=JournalEntry/BookDate ge 2025-07-01T00:00:00Z and JournalEntry/BookDate lt 2025-08-01T00:00:00Z&$count=true` | **200**, `@odata.count 116` — datumbereik op de navigatie werkt (`Z` verplicht); `… eq 2025-07-15T00:00:00Z` gaf 200 mét lege `value` (gelijkheid op een DateTimeOffset treft niets, ook al staat het document op 2025-07-15) |
| h | `GET JournalEntryLines?$filter=JournalEntry/DocumentType eq 11 and …` | **400** `{"Message":"A binary operator with incompatible types was detected. Found operand types 'Reeleezee.DTO.DocumentType' and 'Edm.Int32' for operator kind 'Equal'."}` — enum-literal nodig (vorm `Reeleezee.DTO.DocumentType'11'` niet getest; vgl. "Voorraad-uitstroom STAP-0" over `Status`) |

**Conclusies (bindend voor `app/migratie/rlz_bron.py`, blok 7c):**
1. **Regels + kop per document = de DOCUMENT-vorm** `{collectie}/{id}?$expand=DocumentLineList($expand=Account,TaxRate)`
   — één call per geboekt document, geeft óók `BookDate` (dat op de PurchaseInvoices-collectie ontbreekt). Terugval
   `…/{id}/Lines` alleen waar de Help-lijst 'm kent (PurchaseInvoices, SalesInvoices, BankMutationDirectBookings);
   **voor ManualJournals is er geen terugval**. Een document zonder leesbare regels = rapport "ONVOLLEDIG — niet
   doorrekenen". De collectie-vorm `DocumentLineList(…)` wordt niet meer gevraagd (stil genegeerd = nutteloos).
2. **Er bestaat GEEN koppeling journaalregel ↔ document in de RLZ-API.** `JournalEntry` draagt `id/BookDate/DocumentType/
   EventID(soortcode)`, het document kent geen JournalEntry-navigatie. De RLZ-kolom van de saldibalans blijft de som per
   grootboek uit JournalEntryLines; de boekdatum per document komt van de document-vorm; per DocumentType vergelijkt het
   rapport het aantal journaalposten met het aantal geboekte documenten (volledigheidstoets zonder koppeling).
3. `rlz-lezen` kan sinds 7c een record lezen (recordpad → alleen `$expand`); de eerste échte lezing van
   `ManualJournals/{id}?$expand=DocumentLineList(…)` is onderdeel van de nameting ná deploy (`vgg_blok7_nameting.sh c`,
   meetlat "0 document(en) zonder leesbare regels") — óf los: `nameting.sh rlz-lezen --administratie Vastgoedgroep --pad
   "ManualJournals/bfae5951-fa8e-4329-ad1d-9c8f228fa8b6" --expand "DocumentLineList(\$expand=Account)"`.
...
## Activa-module — STAP-0 16-09 (opdracht Peter 16-09 "MVA bijhouden en afschrijving automatiseren"; lees-only via `nameting.sh rlz-lezen` + publieke Help-pagina's; administraties Kempen Facilities B.V., Universal Steigerbouw B.V., Administratiekantoor Nijenhuis C.V.)

Doel: vaststellen wat de RLZ-API rond `FixedAssets` biedt vóór er iets wordt ontworpen. GEEN actie uitgevoerd, GEEN PUT. Publieke
Help-pagina's (`https://apps.reeleezee.nl/api/v1/Help/Api/<METHODE>-adminId-<Collectie>[-id]`) geven het DTO-model zonder login.

| # | Route / bron | Uitkomst |
|---|---|---|
| a1 | Routelijst (`GET /Help`, 2.141 routes) | Activa-routes: `GET/PUT/DELETE FixedAssets[/{id}]`, `GET/POST FixedAssets/{id}/Actions`, `GET FixedAssets/{id}/DocumentTaskHistory`, `GET Products/{id}/FixedAssets`, enumeraties `AssetTypes`, `AssetMutationTypes`, `DepreciationBaseMethods`, `DepreciationMethodHeaders[/{id}]`, `ActionKinds`. Géén aparte "Depreciate"-route: afschrijven loopt (als actie) via `POST FixedAssets/{id}/Actions {Type: <ActionKind>}` — zelfde vorm als 17/19 op documenten. |
| a2 | `PUT {adminId}/FixedAssets/{id}` body-DTO (Help) | Een FixedAsset is een DOCUMENT (DocumentStatus, ReceiptNumber, Reference, Entity, Date, DueDate, Uploads, PaymentReferenceList) mét activa-velden: `BalanceAccount` (Account — activarekening), `DepreciationAccount` (Account — cumulatieve afschrijving/kosten), `DepreciationMethod` (DepreciationMethodHeader), `NumberOfMonths`, `FirstDepreciationMonth`, `FirstDepreciationYear`, `LiquidationValue` (restwaarde), `OriginalPurchaseDate`, `OriginalTotalAmountPurchase`, `PurchaseDate`, `TotalAmountPurchase`, `TotalNetAmount`, `TotalVatAmount`, `TotalPayableAmount`, `CurrentBookValue`, `CurrentDepreciationValue`, `CalculatedDepreciationAmount`, `BaseInvoiceAmount`, `BaseRemainingAmount`, `HasSales`, `IsArchived`, `Stock`, `Type` (AssetType), `InvoiceReference`, `FixedAssetMutationList` (Collection of FixedAssetMutation), `JournalEntryList` (Collection of JournalEntry), `UploadList`. **Er is geen veld dat naar een PurchaseInvoice-regel verwijst**; de natuurlijke haak vanuit ons boekpad is `InvoiceReference`/`Reference` + `Entity` + bedrag (en ons eigen `activum_koppeling`-record). Schrijfvorm: PUT met client-GUID zoals elk RLZ-document (niet getest). |
| a3 | `GET {adminId}/DepreciationMethodHeaders?$expand=DepreciationBaseMethod` (Kempen Facilities) | **200, 20 rijen**: "Lineair 1 jaar" … "Lineair 50 jaar" (`NumberOfMonths` 12 … 600, `DepreciationBaseMethod` 1), "Vaste Ronde bedragen 2 jaar" (24, basismethode 4), "Waardevast" (12, basismethode 0). Dit zijn RLZ-brede standaardmethoden (zelfde set per administratie te verwachten) — de defaults per rekeningcategorie uit het ontwerp mappen 1-op-1 op "Lineair N jaar". |
| a4 | `GET {adminId}/AssetTypes`, `AssetMutationTypes`, `DepreciationBaseMethods`, `ActionKinds` | **404** onder het administratie-prefix — deze enumeraties zijn ROOT-only (zoals `Administrations`/`DocumentStatuses`). `rlz-lezen` prefixt altijd de administratie; lezen vereist een `--root`-vlag (lees-only uitbreiding, nog niet gebouwd) — de letterlijke waarden staan daarom nog niet vast. Bekend uit a3: DepreciationBaseMethod-id's 0 (waardevast), 1 (lineair), 4 (vaste ronde bedragen). |
| a5 | `GET {adminId}/FixedAssets?$count=true&$top=5` Kempen Facilities | **200, `@odata.count` 0** — geen activa in het register. |
| a6 | idem Universal Steigerbouw | **403 `{"Message":"Actie niet toegestaan bij huidige gebruikersrechten"}`** — de webservice-login van Universal heeft GEEN recht op de activamodule. Gevolg voor de bouw: `FixedAssets` moet in de rechten-probe/leesroutes (`app/rlz/leesroutes.py`) als aparte, niet-blokkerende uitkomst ("activa niet leesbaar — recht ontbreekt"), nooit stil. |
| a7 | `GET {adminId}/AdministrationSettings?$expand=Entity,StandardBusinessIdentification,DefaultTaxRate` Kempen Facilities | **200, 1 rij**: `CompanyName`, `ChamberOfCommerceNumber` 90425405, **`FixedAssetAlertAmount` 450.0** (= de RLZ-activeringsdrempel, exact de wettelijke € 450 — bron voor de instelling in het ontwerp), `StartDateFiscalYear`, `FirstBookDate`, `DefaultTaxRate`, `StandardBusinessIdentification` (SBI). Géén btw-nummer van de administratie in dit DTO. |
| a8 | `JournalEntryLines?$expand=Account,JournalEntry` | `Account.IsFixedAssetAccount` (bool) staat op élke regel én op `Ledgers` (STAP-0 14-09 punt 4) — de MVA-vlag komt dus zonder eigen lijst uit de bron; `JournalEntry.EventID` is de soortcode (13/14-09) — de EventID van een afschrijvingspost is pas te lezen op een administratie MÉT activa (open punt). |
| a9 | `GET {adminId}/Ledgers?$filter=IsFixedAssetAccount eq true` Universal Steigerbouw | **200, 21 rijen — de vlag is GEEN zuivere MVA-markering**: náást 0101 Gebouwen en terreinen, 0107 Kantoorinventaris, 0111 ICT-apparatuur, 0113 Computersoftware (AccountType 3) staan óók 1100–1110 Voorraad/Onderhanden werk (type 3) én 7000–7050 Inkopen/Inhuur (AccountType 2, kosten). `IsFixedAssetAccount` is RLZ's rekening-optie "gebruiken voor vaste activa" en is bij Universal breed aangevinkt. Ontwerpgevolg: MVA-rekening = `IsFixedAssetAccount` ÉN `AccountType` 3 ÉN code in de RGS-activa-reeks (0xxx) — nooit de vlag alleen. |
| a10 | `GET {adminId}/FixedAssets?$count=true` Administratiekantoor Nijenhuis C.V. | **200, 0 activa.** Drie gelezen administraties: 0 / 0 / geen recht — de nulmeting "wél op de balans, niet in het register" vergt de lees-only CLI `activa-nulmeting` over alle 76 (meetrecept in het rapport; ná deploy). |

**Conclusies voor het ontwerp (`docs/ONTWERP_ACTIVA_MVA.md`):** (1) RLZ heeft een volwaardig activaregister met methode/termijn/restwaarde/
rekeningen en een actie-route; (2) er is géén regel-koppeling naar de inkoopfactuur — koppelen op `InvoiceReference` + Entity + bedrag
en een eigen koppelrecord; (3) de activeringsdrempel staat al in RLZ (`FixedAssetAlertAmount`); (4) rechten zijn per administratie
...
| # | Route / bron | Uitkomst |
|---|---|---|
| b1 | `activa-nulmeting` over alle administraties (executie `rlz-reconciliatie-hrtzq`, `verkenning/nameting-activa-nulmeting-21-09.txt`) | 78 administraties, 75 mét MVA-rekeningen (vlag ÉN type 3 ÉN 0xxx: 344 rekeningen), **51 activa in 7 registers** (Pilates Bloom 20, Zilver Beheer 13, Mantelzorgwoningen Midden Nederland 11, Belastingbutler 3, Beauty by Tessa Elst 2, Dimo Living & Styles 1, L.H.G. Holding 1), 1 module-regel op een MVA-rekening (€ 45,67). **403 "recht ontbreekt"** bij Universal Steigerbouw (4 MVA-rekeningen, 0113 € 9.690) én Rubicon Investments (28 MVA-rekeningen, 0113 Verhuurmateriaal € 5.225.769,67, 403 regels). Odoo-administraties zichtbaar overgeslagen (2). Grote saldi zonder registerpost: Meyer 0101 € 2.440.000, Van Rooijen/Schaalje 19 pand-rekeningen (0120–0134), A.Y. Holding 0101 € 165.729. |
| b2 | `GET {adminId}/FixedAssets?$top=2&$expand=JournalEntryList,FixedAssetMutationList,DepreciationMethod,BalanceAccount,DepreciationAccount,Type` Pilates Bloom | **200.** Per activum: `id`, `ReceiptNumber` ("1", "2" — eigen volgnummerreeks), `Description`, `InvoiceReference` (null), `TotalAmountPurchase`/`OriginalTotalAmountPurchase`, `PurchaseDate`/`OriginalPurchaseDate`, **`CurrentBookValue` + `CurrentDepreciationValue`** (10.000 op 18-11-2024 → 6.326 / 3.674 op 21-09-2026 = 22 maanden × 166,67: RLZ rekent de lineaire afschrijving zélf), `Type` 1, `Status` 2, `DocumentType` 20, `DepreciationMethod {id, NumberOfMonths 60, Description "Lineair 5 jaar", DepreciationBaseMethod 1}`. **`JournalEntryList`, `FixedAssetMutationList`, `BalanceAccount`, `DepreciationAccount` komen op de COLLECTIE niet mee** (collectie-expand stil genegeerd, zelfde gedrag als 7c) — de record-vorm `FixedAssets/{id}?$expand=…` is de STAP-0 van fase 2 (of de afschrijving als journaalpost geboekt staat, is hier dus nog niet gezien). |
| b3 | `GET AssetTypes` / `AssetMutationTypes` / `ActionKinds` via `rlz-lezen --root` | `AssetTypes`: 1 Fixed, 2 Stock. `AssetMutationTypes`: 1 Purchase (Inkoop), 3 Sale (Verkoop), 4 Revaluate (Herwaarderen), 5 Depreciate (Afschrijven), 6 Manual (Handmatig). `ActionKinds` top 5 = CustomerTaxDeposit*/CredentialsChangePassword (lange lijst, `$top` nodig; de activa-actiekinds zijn nog niet gelezen). |
| b4 | `GET {adminId}/AdministrationSettings?$top=1` Pilates Bloom | **200**, `FixedAssetAlertAmount` 450.0 (net als Kempen Facilities, a7) — de activeringsdrempel komt uit RLZ als die gevuld is. |
| b5 | `GET {adminId}/FixedAssets?$top=3&$expand=JournalEntryList,FixedAssetMutationList` Zilver Beheer | 200, drie activa "Trading company" (7.062,50 / 4.237,50 / 2.825,00, juni 2025) mét CurrentDepreciationValue 708/426/282 — zelfde patroon; geen expand-inhoud. |
| b6 | `GET {adminId}/Receipts?$filter=ReceiptNumber eq 'RLZ-01-00000082'&$expand=DocumentLineList($expand=Account,Project),PaymentReferenceList,Entity` Vastgoedgroep Nederland (VGG-bewijspaar) | 200, één treffer: `id` 8b079e5c…, € 400.000,00, Date 19-03-2026, Status 3, `InvoiceNumber` 171384, Entity "O.N." (EntityKind 1); **regels en betaalkoppelingen komen op de collectie niet mee** (zelfde 7c-gedrag; `Receipts/{id}` bestaat niet → 404). Pand-identificatie loopt dus via de eigen bank-cache: `bank_mutatie.brondata.TransactionId` "00112" = 20-03-2026 € 52.142,09 Ouwerkerk Notariaat "Betreft: schoffelstraat 29 te Purmerend, ons dossier: 2026.079950.01" (leesreplica, `db_lezen.sh`). |

**Conclusies:** (1) fase 1 start op Pilates Bloom B.V. (rijkste leesbare register); (2) een activum aanmaken = `PUT FixedAssets/{client-GUID}` mét
`BalanceAccount`, `DepreciationAccount`, `DepreciationMethod` (Lineair N jaar), `TotalAmountPurchase`, `PurchaseDate`, `LiquidationValue`, `Type` 1 —
respons 204, altijd terug-lezen; (3) RLZ berekent `CurrentDepreciationValue` zelf — het reconciliatiesignaal "afschrijving niet gelopen" toetst
