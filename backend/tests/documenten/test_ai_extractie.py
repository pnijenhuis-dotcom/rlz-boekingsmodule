...
def _fake_extractie() -> AiFactuurExtractie:
    return AiFactuurExtractie(
        ...
        regels=[
            AiRegel(
                omschrijving=_veld("Steigerhout"),
                netto_bedrag=_veld("100.00"),
                btw_bedrag=_veld("21.00"),
                hoeveelheid=_veld("4"),
            ),
            AiRegel(
                omschrijving=_veld("Bezorging"),
                netto_bedrag=_veld("50.00"),
                btw_bedrag=_veld("7.50"),
                hoeveelheid=_veld(None),
            ),
        ],
        bsn_verwijderd=0,
    )
...
def _extractie_detail(administratie_id: uuid.UUID, document_id: uuid.UUID) -> dict | None:
    ...
    for gebeurtenis in detail.gebeurtenissen:
        if gebeurtenis.naar_status == DocumentStatus.TE_CONTROLEREN:
