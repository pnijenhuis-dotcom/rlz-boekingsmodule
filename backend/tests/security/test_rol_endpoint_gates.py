...
def _kantoor_endpoints(aid: uuid.UUID) -> list[tuple[str, str]]:
    ...
    return [
        ...
        ("GET", "/crediteuren/dubbelen/stand"),  # KPI-teller twijfelclusters (B13 07-09)
        ("POST", "/crediteuren/dubbelen/auto-afhandelen"),  # auto-afhandeling eenduidige clusters (B13 07-09)
        ("GET", "/crediteuren/afhandelingen"),  # log afgehandelde clusters + terugdraai-ingang (B13 07-09)
        ("POST", f"/crediteuren/afhandelingen/{DUMMY_ID}/terugdraaien"),  # terugdraaien mét reden (B13 07-09)
        ("GET", "/crediteuren/opruimlijst.csv"),  # optionele RLZ-opruimlijst-export (B13 07-09)
        ...
        ("POST", f"/administraties/{aid}/odoo/overstap/voorbereiden"),  # mapping-voorstel overstap (blok A 04-09)
        ("GET", f"/administraties/{aid}/odoo/mapping"),  # rekening-mapping RLZ→Odoo (blok A 04-09, beheerder-only)
        ("PUT", f"/administraties/{aid}/odoo/mapping/grootboek/{DUMMY_ID}"),  # mapping-correctie (blok A 04-09)
