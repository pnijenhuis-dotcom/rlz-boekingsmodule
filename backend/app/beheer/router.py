    ...
    return schemas.AdministratieInstellingenLijstDto(
        administraties=[
            schemas.AdministratieInstellingenDto(
                ...
                odoo_api_gebruiker=r.odoo_api_gebruiker,
                odoo_api_key_verloopt_op=r.odoo_api_key_verloopt_op,
                odoo_probe_groen=r.odoo_probe_groen,
                odoo_url=r.odoo_url,
                odoo_probe_op=r.odoo_probe_op,
                odoo_alleen_lezen=r.odoo_alleen_lezen,
                odoo_voorraad_knip_datum=r.odoo_voorraad_knip_datum,
                odoo_overgangsdatum=r.odoo_overgangsdatum,
...
def duplicaat_autoafvoer_platform_ophalen(
    actor: CurrentGebruiker = Depends(require_beheerder),
) -> schemas.DuplicaatAutoafvoerDto:
    return schemas.DuplicaatAutoafvoerDto(ingeschakeld=service.haal_duplicaat_autoafvoer_platform_op())
...
def duplicaat_autoafvoer_platform_zetten(
    invoer: schemas.DuplicaatAutoafvoerDto, actor: CurrentGebruiker = Depends(require_beheerder)
) -> schemas.DuplicaatAutoafvoerDto:
    """Platformbrede noodrem duplicaat-auto-afvoer (blok A1 04-09, migratie 0109) — Beheerder-only, default
    AAN; zelfde patroon als "Boeken platformbreed". Het pad zelf voert alleen af bij een HARDE match
    (app/documenten/duplicaat_afvoer.py)."""
    try:
        ingeschakeld = service.zet_duplicaat_autoafvoer_platform(actor_id=actor.id, ingeschakeld=invoer.ingeschakeld)
    except service.BeheerFout as exc:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(exc)) from exc
