...
class ProjectRijData:
    """Eén projectrij in het weekgrid (alleen actieve projecten, mockup-norm)."""
...
def planning_overzicht(
    *,
    ...
    with scoped_session(administratie_id, actor_id=actor_id) as session:
        ...
        # Projectrijen: alle actieve projecten van de administratie (mockup: afgeronde
        # projecten verdwijnen uit de lijst), mét spec-samenvatting.
        projecten = list(
            session.scalars(
                select(ProjectCache)
                .where(
        ...
        for project in projecten:
            spec = session.get(ProjectSpecificatie, (project.id, administratie_id))
