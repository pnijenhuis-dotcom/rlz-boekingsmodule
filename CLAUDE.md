...
- **Vastgoedgroep Nederland → Odoo, run 2 blok 8 (15-09; "go" Peter 15-09: SCHRIJF a + derde meting; geen migratie):** expliciete rekeningmapping `app/migratie/rekening_mapping.py` — RLZ 1012 Betalingen onderweg → Odoo Outstanding Payments van het bankdagboek (lees-only: payment-method-line → company-default → KLIKPUNT, nooit de bankrekening zelf; in de bankgroep, geen RJ-220-rol), RLZ 1001 als SCHRIJF-b-markering (memoriaal-1001 → outstanding/suspense, bankgroep verwacht rood), voorstel-kolom per ongemapte rekening, replay leest de Odoo-rekeningen zelf via de doelkoppeling, `SCHRIJF a` idempotent; STAP 2–4 (plan → SCHRIJF a → derde meting) als vervolg-opdracht via de inbox ná deploy (push = Stop-hook) — zie BESLISSINGEN "VASTGOEDGROEP NEDERLAND → ODOO — RUN 2 BLOK 8: SCHRIJF a + DERDE METING (Peter 15-09)".
