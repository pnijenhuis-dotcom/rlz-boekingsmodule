...
- **Productie-nametingen structureel (blok 7 vervolgrun 10-09 avond; voorbereiding, niets aangemaakt):** twee routes uitgeschreven in `docs/GCP_UITROL.md` §F7 — (A) dedicated SA `nameting@` mét custom rol (`run.jobs.run` + `run.jobs.runWithOverrides` — dat laatste zit alleen in het te ruime `roles/run.developer`, daarom custom) job-scoped + `run.viewer`/`logging.viewer`, géén `cloudsql.client`, key in `~/Sleutels`, latere lees-only job/`CLI_ALLOWLIST`; (B) Workspace Google Cloud-sessieduur max. 24 u — Peter kiest — zie BESLISSINGEN "PRODUCTIE-NAMETINGEN STRUCTUREEL — SERVICEACCOUNT OF LANGERE WORKSPACE-SESSIE".
