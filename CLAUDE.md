...
- **Vastgoedgroep Nederland → Odoo, run 1 (D3/D4, 10-09; geen code, geen migratie):** klikbare mockup `mockup/pandenregister.html` (Inzicht › Panden: register / toewijzing met voorstel-kaart + bulk bevestigen / dashboard per pand met overheadsleutel "in handel per maand" — aankoop- en verkoopmaand tellen vol, nul panden = "niet toegerekend"; ontwerpnotities ①–⑧, bouw pas ná akkoord) + lees-only Odoo-verkenning §11 in `verkenning/odoo-verkenning.md` (statement lines, reconciliatiemodellen in 19 zonder `rule_type`, verkoopfactuur notaris `tax_ids=[]`, bewijscyclus op de TESTdatabase; `ODOO_TEST_URL`/`ODOO_TEST_API_KEY` als secrets) — zie BESLISSINGEN "VASTGOEDGROEP NEDERLAND → ODOO — RUN 1: MOCKUP PANDENREGISTER + ODOO-VERKENNING".
