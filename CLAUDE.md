...
- **Kempen-doorbelasting en intercompany** — Tweezijdige motor bron-verkoop + spiegel-inkoop ("Boeken + doorbelasten"), whitelist + doelentiteiten, IC-vlag, tegenboek-pad ná ingediende aangifte, aansluiting KF ↔ doelentiteiten, herkoppeling, intercompany slaat klant-accordering over.
  ...
  5. Factuur-PDF "onvolledig" = cent-verschil (lees-only, Peter 24-09, Lusso 261004): RLZ legt op verkoop `RLZ-01-00002726` én spiegel `RLZ-04-00000614` 6.024,14 vast (btw per tarief over het subtotaal), de motor boekte de regelsom 6.024,15 → uitkomst B; lees-only instrument `doorbelasting-factuur-pdf-toets` (klasse-telling + `--pdf`), fix in `doorbelasting/geld.py` pas ná Peters akkoord; dispatch-onderdeel `doorbelasting-pdf` — zie BESLISSINGEN "DOORBELASTING — FACTUUR-PDF 'ONVOLLEDIG' = CENT-VERSCHIL PER-REGEL-AFRONDING (lees-only, Peter 24-09)".
