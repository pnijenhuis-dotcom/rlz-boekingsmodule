...
- **Werkvoorraad, documentenlijst en controlescherm** — Klantenlijst met tellers → documentenlijst → controlescherm; boekingsgeheugen, vragen, afwijzen, doorloop na boeken, kalenderdag, zoeken/archief.
  ...
  9. Documentlink volgt de soort (BUG 23-09, Van Boxtel): één routefunctie `documentPad` (frontend) + spiegel `deeplink.document_pad` (backend), 17 linkplekken gemigreerd, guard op letterlijke `/documenten/${`-links, redirect op het inkoop-controlescherm voor kassarapport/verkoop/waarborg/verplichting, thread-knop "Document bekijken" + "Naar omzetreview →" — zie BESLISSINGEN "DOCUMENTLINK VOLGT DE SOORT — VRAAG-THREAD OPENDE KASSARAPPORT IN INKOOPSCHERM (23-09)".
  10. Samenvoegen-vinkje (BUG 23-09, Van Rumpt 2025135): ≥ 2 opgeslagen regels = de bron van de samengevoegde regel (Σ netto/btw, één btw-code), scan alleen zonder opgeslagen regels; kan het niet → chip "samenvoegen niet mogelijk: ‹reden›" (`samenvoegen_niet_mogelijk_reden`), nooit stil weg — zie BESLISSINGEN "SAMENVOEGEN — BRON = OPGESLAGEN REGELS, NOOIT STIL WEG (23-09)".
  11. Crediteur-zijpaneel + bewerken (Peter 25-09, FV-14/FV-15): "+ Nieuwe crediteur in RLZ" = niet-modaal `Zijpaneel` naast de factuur (naam/KvK/btw/IBAN/adres uit UBL, chips per veld, geen IBAN = waarschuwing), "Gegevens bewerken…" = `PUT …/crediteuren/{id}` (RLZ-naam + adres fail-open, kenmerk 'handmatig', audit `crediteur_gewijzigd`), IBAN uitsluitend via de vier-ogen-route; meetlat `db-lezen crediteur-mutaties`, dispatch-onderdeel `crediteur-paneel` — zie BESLISSINGEN "CREDITEUR AANMAKEN ALS ZIJPANEEL NAAST DE FACTUUR + CREDITEUR BEWERKEN (Peter 25-09)".
  12. "Open (N)" + échte "Alles (N)" + zoeken over alles (Peter 25-09, FV-20): het kantoorwerk-filter heet "Open" (semantiek/URL `alle` ongewijzigd, `open` synoniem); "Alles" = server-side `groep=alles` (kantoor ∪ wachten ∪ afgehandeld, limit 200/max 500, `totaal`, statuschip per rij, paginabalk); een zoekterm vanaf Open/Alles/binnenkomst zoekt server-side over alles (`q`: leverancier/referentie/bestandsnaam/bedrag), lege treffer benoemt "over alle statussen"; bestaande aanroepen byte-gelijk — zie BESLISSINGEN "DOCUMENTENLIJST — "OPEN (N)" EN EEN ÉCHTE "ALLES (N)" MÉT SERVER-SIDE ZOEKEN (Peter 25-09)".
  13. Tabwissel documentenlijst (FV-18, Peter 25-09): een tabwissel is client-side en start nooit een server-request (bulk-balk krijgt de administraties van het ouder), de 3-s-poll tikt vast, stapelt niet en rendert niet bij een ongewijzigd antwoord, lijst- en zij-fetches op een `AbortController` (laatste lading wint); meetinstrument `frontend/scripts/tabwissel_meting.sh` (harnas `?docs=N&tabwissel=K`, CDP mét échte klok), dispatch-onderdeel `tabwissel` — zie BESLISSINGEN "TABWISSEL DOCUMENTENLIJST — GEMETEN, FETCH PER WISSEL EN POLL-STORM WEG (Peter 25-09)".
  14. Comfort controlescherm (Peter 25-09, FV-05/07/08/10/11/12/13): bijlageverwijzing van de staart van de automatische kop-omschrijving gestript (chip "ingekort"), project/btw-code op factuurniveau → alle regels mét tijdlijn "kop → regels", rekenexpressies in bedragvelden (eigen parser, geen eval), splitter-default 42/58 + pointer capture, knop "Verdelen over projecten" mét de standaardsleutel uit de historie van de administratie (nooit hardcoded), periode-chip "van … tot …" en terugval letterlijk "aanname" — zie BESLISSINGEN "COMFORT CONTROLESCHERM — BIJLAGEVERWIJZING, KOP → REGELS, REKENEN IN BEDRAGVELDEN, SPLITTER, VERDELEN-KNOP, PERIODE VAN–TOT (Peter 25-09)".
...
- **Administraties, RLZ-/Odoo-koppelingen, sync en instellingen** — Instellingen › Administraties (archiveren, nooit verwijderen), wizard mét rechten-probe, eerste sync, RLZ-check, groepen, administratienaam volgt de bron, Odoo-koppelwizard, terugkerend-signaal, verplaatsen.
  ...
  5. Odoo taal-poort (Peter 24-09, Bonte Hoeve): `OdooClient.call` draagt `context.lang = nl_NL` op élke call (expliciete lang wint); hersync = de nachtelijke `sync-alles` of CLI `odoo-stamgegevens-sync` (schrijvend, job-image); meetlat `db-lezen grootboek-taal`, dispatch-onderdeel `odoo-taal` — zie BESLISSINGEN "ODOO — TAAL NL_NL IN ÉLKE CALL (Peter 24-09)".
  6. Dearchiveren Odoo-administratie (BUG Peter 24-09, Recreatief Vastgoed Nederland): `HeractiveerPort` via de 0016-registry — Reeleezee = nieuwe login + probe, Odoo = bestaande koppeling + versleutelde sleutel opnieuw geprobed (company terug-gelezen moet gelijk zijn), body optioneel, dialoog zonder loginvelden, archiveren laat de Odoo-sleutel versleuteld staan (`odoo_sleutel_behouden`), wizard toont "gearchiveerd — dearchiveer ‹naam›"; Peter dearchiveert 8ea9d28b (company 13) zelf ná deploy; dispatch-onderdeel `dearchiveren-odoo` — zie BESLISSINGEN "DEARCHIVEREN ODOO-ADMINISTRATIE — BACKEND-BEWUST VIA DE PORT, GEEN REELEEZEE-LOGIN (Peter 24-09)".
...
- **Btw: codes, defaults, verlegd, buitenland** — Btw-code uit de scan en het factuurtotaal, defaults uit de RLZ-grootboekrekening en de eigen historie, verlegd-herkenning (vermelding, kolomcode, onderaannemer), buitenland-signaal, verlegd-tarief deterministisch.
  ...
  7. BUA-jaarrapport i.p.v. kenmerk (besluit Peter 24-09 "nee standaard 21 % btw aanhouden"): geen bulk-zetting; lees-only CLI `bua-jaarrapport --jaar [--rlz]` (module + bank, RLZ alleen GET; kantine/sponsoring apart, kolom "correctie laatste aangifte (voorstel)", € 227-drempel = LET-OP-tekst), bevinding `bua_correctie_open` (meten) vanaf 1 december mét "Rapport openen", lees-only blok op Boeken & AI; kenmerk 0163 blijft per rekening; dispatch-onderdeel `bua-jaarrapport` — zie BESLISSINGEN "BUA — JAAREINDE-RAPPORT I.P.V. KENMERK (Peter 24-09)".
  8. Btw herrekent óók bij nettowijziging (FV-09, Peter 25-09): een geladen/AI-gelezen btw-bedrag is geen mens-keuze meer (`btwHandmatig` = alleen zelf getypt), élke nettowijziging herrekent cent-exact via de `regelsom.ts`-spiegel (ook samengevoegd), mens-btw wint tot het netto wijzigt → dan chip "btw herrekend (netto gewijzigd)" + tijdlijnregel `btw_herrekend` mét `aanleiding: netto`; tarief onbekend = chip, veld blijft; check + marge 18-09 ongewijzigd; meetlat `db-lezen btw-herrekend`, dispatch-onderdeel `btw-netto` — zie BESLISSINGEN "BTW-BEDRAG VOLGT HET TARIEF ÓÓK BIJ NETTOWIJZIGING (Peter 25-09)".
  9. Factuurdatum in een ingediende aangifteperiode (Peter 25-09, FV-16 aangepaste vorm): ORANJE check via `app/rlz/aangifte.py` op de factuurdatum (extern deel, gecachet), handeling "Boeken (btw in volgend tijdvak)" = tijdlijn-notitie + audit `aangifteperiode_bevestigd`, boeken zonder keuze mag mét tijdlijnregel, autoboek-pad weigert; doorbelasting = LET-OP in de preview als één kant ingediend is; meetlat `db-lezen aangifteperiode-bevestigingen`, dispatch-onderdeel `aangifteperiode` — zie BESLISSINGEN "FACTUURDATUM IN EEN INGEDIENDE AANGIFTEPERIODE — ORANJE CHECK, GEEN BLOKKADE (Peter 25-09)".
...
- **Verplichtingen/offertes, projecten, projectverdeling, contract-ontleding en voorraad** — Documenttype `verplichting` + factuur↔offerte-match (nooit blokkerend), projectenmodule en projectcode-generatie, Inzicht › Projecten/Projectverdeling, pro rato, contract-ontleding auto-first, mini-voorraad en voorraad-aansluiting (mi-schema, nooit RLZ-writes).
  ...
  9. Project-bronvolgorde (Peter 25-09, FV-02): factuur/werknummer > klant-loze cachecode in het administratie-formaat (`match.ProjectcodeFormaat`, uit de projectcache — nooit hardcoded; óók UBL-`cbc:Note`) > geheugen als LAATSTE bron mét chip "voorstel uit historie"; factuur noemt een ander nummer = niets invullen (`factuur_conflict`, kies zelf), afgesloten alleen bij exacte verwijzing, autoboek weigert bij conflict; meetlat `db-lezen project-prefill-herkomst`, dispatch-onderdeel `project-bronvolgorde` — zie BESLISSINGEN "PROJECT-BRONVOLGORDE — FACTUUR EN CACHECODE VÓÓR HET GEHEUGEN, GEHEUGEN ALTIJD ZICHTBAAR (Peter 25-09)".
...
- **Activa / MVA** — AKKOORD Peter 21-09 ("activa, JA", alle defaults §8) op `docs/ONTWERP_ACTIVA_MVA.md`; fase 1 GEBOUWD 21-09 (migratie 0168): register-lezer RLZ `FixedAssets` + probe (403 = zichtbaar "recht ontbreekt"), detectie bij boeken (`grootboekrekening.is_activa` + activeringsgrens 450/RLZ `FixedAssetAlertAmount`), kaart "Activum aanmaken?" op het controlescherm, activum in RLZ ná boeken (mens bevestigt; opt-in automatisch per administratie, default UIT), reconciliatieblok `activa` in `meten`; fiscale toetsing zonder zelf rekenen — zie BESLISSINGEN "ACTIVA / MVA — FASE 1 GEBOUWD (Peter 21-09)".
  ...
  3. Gemeten 22-09: sync (344/75, grens, probe) + blok `activa` (75 getoetst, 22 in `meten`) + kaart-route werken in productie JA; kaart/aanmaken niet gemeten (geen MVA-factuur in de module = klikpunt); een lees-only reconciliatie schrijft óók geen probe-stand (bijvangst gefixt), meetlat `db-lezen activa-stand`; gemeten 23-09: probe-fix JA (75/75 ongewijzigd ná lees-only run), kaart gebruikt op BLOw maar activum in RLZ NEE (geen afschrijvingsrekening → BUG-opdracht 24-09) — zie BESLISSINGEN "ACTIVA / MVA — FASE 1 GEBOUWD (Peter 21-09)".
